...
def dict_count(bag):
    """Counts the frequency of identical items in a dictionary
    returns "item: count"""
    dict_of_counts = {}
    for item in bag:
        dict_of_counts[item.lower()] = bag.count(item.lower())
    return dict_of_counts

def maketotal(dict1):
    """Finds the total occurances of an item between two dictionaries"""
    total = 0
    for item in dict1:
        total += dict1[item]
    return total
...
def jaccard(dict1, dict2):
    """Computes the jaccard similarity of two dictionaries"""
    intersection = {}
...
def all_similarity(bags):
    """Returns a sorted dict of Jaccard similarities between bags, 
    from least similar to most similar."""
    
    # Convert bags to count dictionaries
    count_bag = {i: dict_count(bag) for i, bag in enumerate(bags)}
    
    jaccard_dict = {}
    n = len(count_bag)

    # Compare only unique pairs (i < j), no duplicates or self-comparison
    for i in range(n):
        for j in range(i + 1, n):
            key = f"bag{i},{j}"
            jaccard_dict[key] = jaccard(count_bag[i], count_bag[j])

    # Sort by similarity value
    return dict(sorted(jaccard_dict.items(), key=lambda item: item[1]))

def main():
    """Prints the Jaccard similarity of the documents from least to most"""
    docs=[]
    ...
    bags = create_bags(docs)
    results = all_similarity(bags)
    print(results)
